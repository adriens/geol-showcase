...
\begin{itemize}
    \item \textbf{5 Supported Versions:} PostgreSQL versions 14-18 are actively maintained with EOL dates ranging from 2026 to 2030.
    \item \textbf{Fresh EOL Alert:} PostgreSQL 13 recently reached end-of-life on November 13, 2025, joining versions 9.6-12 as unsupported.
    \item \textbf{Security Gap:} Unsupported versions contain \textbf{2-10× more vulnerabilities} than supported versions, with critical CVEs present only in EOL versions.
    \item \textbf{Patch Impact:} Minor updates are crucial - PostgreSQL 18.1 reduced vulnerabilities by 22\% compared to 18.0 (204 → 159 total vulnerabilities).
    \item \textbf{Critical Recommendation:} Migrate immediately from any version $\leq$13 to version $\geq$14. The security risk of unsupported versions is unacceptable for production environments.
\end{itemize}

\textbf{Risk Profile Summary:}
\begin{itemize}
    \item $\checkmark$ \textbf{Low Risk:} Versions 14-18 (0-1 critical, 6-17 high vulnerabilities)
    \item $\times$ \textbf{High Risk:} Versions 9.6-13 (7-10 critical, 70-97 high vulnerabilities)
\end{itemize}
...
\addplot[gantt-eol, line width=6pt] coordinates {(2017,2) (2022.86,2)}; % 10: 2017-2022.86
\addplot[gantt-eol, line width=6pt] coordinates {(2018,3) (2023.86,3)}; % 11: 2018-2023.86
\addplot[gantt-eol, line width=6pt] coordinates {(2019,4) (2024.89,4)}; % 12: 2019-2024.89
\addplot[gantt-eol, line width=6pt] coordinates {(2020,5) (2025.87,5)}; % 13: 2020-2025.87

% Supported versions
\addplot[gantt-supported, line width=6pt] coordinates {(2021,6) (2026.87,6)}; % 14: 2021-2026.87
\addplot[gantt-supported, line width=6pt] coordinates {(2022,7) (2027.87,7)}; % 15: 2022-2027.87
\addplot[gantt-supported, line width=6pt] coordinates {(2023,8) (2028.86,8)}; % 16: 2023-2028.86
\addplot[gantt-supported, line width=6pt] coordinates {(2024,9) (2029.86,9)}; % 17: 2024-2029.86
\addplot[gantt-supported, line width=6pt] coordinates {(2025,10) (2030.87,10)}; % 18: 2025-2030.87

% Current date marker
\addplot[black, dashed, line width=1pt] coordinates {(2026,0) (2026,10.5)};
\node at (axis cs:2026,10.5) [above] {\small Today};

\legend{EOL/Unsupported, Supported}
\end{axis}
\end{tikzpicture}
...
% Add value labels
\node at (axis cs:Critical,9.6) {\tiny 10};
\node at (axis cs:High,9.6) {\tiny 97};
\node at (axis cs:Critical,12) {\tiny 9};
\node at (axis cs:High,12) {\tiny 70};
\node at (axis cs:Medium,12) {\tiny 100};
\node at (axis cs:High,18) {\tiny 17};
...
\subsection{Recommended Upgrade Paths}

PostgreSQL's 5-year support policy means careful planning is essential. Here are tested migration strategies:

\textbf{For Production Systems on EOL Versions:}

\begin{enumerate}
    \item \textbf{Assess Current State}
    \begin{itemize}
        \item Run \texttt{pg\_dump} to ensure backup capability
        \item Document custom configurations and extensions
        \item Review application compatibility with target version
    \end{itemize}
    
    \item \textbf{Choose Target Version}
    \begin{itemize}
        \item Minimum: PostgreSQL 14 (EOL 2026) - shortest migration path
        \item Recommended: PostgreSQL 16 or 17 (EOL 2028-2029) - better long-term support
        \item Latest: PostgreSQL 18 (EOL 2030) - maximum future-proofing
    \end{itemize}
...
pg_restore -U postgres -d mydb mydb.dump
    ...
    \item \textbf{Testing Checklist}
    \begin{itemize}
        \item Verify all queries execute correctly
        \item Test application endpoints under load
        \item Validate custom functions and stored procedures
        \item Check extension compatibility (\texttt{pg\_upgrade --check})
    \end{itemize}
\end{enumerate}

\textbf{Automation Best Practices:}
...
\addplot[fill=lightorange] coordinates {(Critical,4) (High,30) (Medium,68) (Low,102)};
\addplot[fill=lightgreen] coordinates {(Critical,1) (High,17) (Medium,39) (Low,102)};
\addplot[fill=lightgreen] coordinates {(Critical,1) (High,17) (Medium,39) (Low,102)};
\legend{Postgres 18.0, Postgres 18.1, Postgres 18.2}
\end{axis}
\end{tikzpicture}
\caption{Comparison of vulnerabilities detected by \texttt{trivy} for PostgreSQL 18.0, 18.1 and 18.2.}
